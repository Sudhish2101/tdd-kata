...
describe('AppService', () => {
    ...
    describe('calculateString', () => {
        it('should return 0 for an empty string', async () => {
            ...
            expect(await appService.calculateString(calculateStringDto.numbers)).toBe(0);
        });
        ...
        it('should return the sum for a string of comma-separated numbers', async () => {
            ...
            expect(await appService.calculateString(calculateStringDto.numbers)).toBe(6);
        });
        ...
        it('should support custom delimiters', async () => {
            const calculateStringDto: CalculateStringDto = { numbers: '//;\n1;2' };
            expect(await appService.calculateString(calculateStringDto.numbers)).toBe(3);
        });

        it('should throw an error for negative numbers', async () => {
            const calculateStringDto: CalculateStringDto = { numbers: '1,-2,3' };
            await expect(appService.calculateString(calculateStringDto.numbers)).rejects.toThrow('Negative numbers not allowed: -2');
        });

        it('should throw an error listing all negative numbers', async () => {
            const calculateStringDto: CalculateStringDto = { numbers: '1,-2,-3,4' };
            await expect(appService.calculateString(calculateStringDto.numbers)).rejects.toThrow('Negative numbers not allowed: -2, -3');
        });
        ...
        it('should return the correct sum for large input strings', async () => {
            ...
            expect(await appService.calculateString(calculateStringDto.numbers)).toBe(1000);
        });
    });
});
